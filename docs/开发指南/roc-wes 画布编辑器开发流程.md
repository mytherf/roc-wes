...
> 注册节点组件：
> - 创建 Vue 组件： 如 CustomCard.vue
> - 在 `registry.ts`中导入组件：如 import CustomCard from './CustomCard.vue'
> - 在 `registry.ts` 的 `nodeRegistry` 加一项（shape → 组件），

> 使用节点组件：
> - 在 `nodeTemplates.ts` 加一条组件模板配置。
> - 在 `Sidebar.vue`中导入组件列表：'@/components/nodes/nodeTemplates'
...
Sidebar.vue → @mousedown="handleDragStart($event, item)"
            → buildNodeConfig(item)              // nodeTemplates.ts
...
2. 构建 `data`：
   - `buildData()` 优先（复杂节点，如货架：初始货格需随机生成）；
   - 否则 `data` 静态对象深拷贝 / 函数形式返回全新对象（避免引用共享）。

> v3 起不再自动生成 pointId、不再注入 binding（旧演示时代的默认点ID 已移除，
> `PointIdGenerator` 已删除）；点组一律由用户在属性面板「数据绑定」页录入。
> 复制粘贴（Ctrl+C/V/D）同样剥离源节点的 pointId/binding，复制出的节点独立起步。
...
`cell:removed` 触发 `onNodeRemoved`：`unbindNodeData`（取消订阅）→ 清除路线覆盖层。
...
1. `resolveBindingPoints(binding)` 归一化点组：取 `points[]`（去重去空），
   点组地位平等；驱动渲染的画面点由 `resolveDisplayPointId` 解析
   （`binding.display.pointId` 有效时用之，否则回落首个点组，旧存档无 display 行为不变）；
...
画面点（display.pointId，缺省回落首个点组）额外写入 `node.data.value`（附 `_timestamp` / `_quality`）驱动节点渲染 →
...
（点组列表：点 ID + 点名称 + 转换函数 + 备注成组；点组从零开始，仅「添加/导入」产生，全部可删、地位平等；
画面点 `displayPointId` 随节点切换回填/重置，显式指定且在点组中才生效，缺省回落首个点组；
...
- **有点组即提交**（`sourceId` 允许后补）：点位是用户录入的设计数据，
  若要求"点组 + 数据源同时具备"才写入，未选数据源时录入的点位会在切换节点时丢失；
  无 sourceId 的绑定运行期不订阅，节点保持静态值，语义安全；
- **画面点写入从简**：`display` 仅当显式指定、在点组中有效且不等于首个点组时才写入
  （缺省回落首点，保持存档精简；画面点所在组被删/失效时自动顺延首个点组）；
...
```mermaid
graph TD
    A["Sidebar: mousedown 模板项"] --> B["buildNodeConfig: 形状/尺寸/data 构建"]
