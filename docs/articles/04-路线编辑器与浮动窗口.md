...
路线存储在独立的 Pinia store（`routeStore`）中，持久化到 `localStorage`（key: `roc-wes-routes`）。加载时做了向后兼容处理——旧数据缺少 `segments`、`smooth` 字段时自动补默认值。
...
**上一篇**：[（三）多协议数据接入体系：7 种工业协议如何统一抽象](03-多协议数据接入体系.md)
