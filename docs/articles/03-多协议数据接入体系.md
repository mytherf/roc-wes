...
浏览器开发环境（`npm run dev`）没有 Tauri 运行时，工业协议回退到旧的 `GatewayService` 抽象基类 + WS 桥接实现（S7Service / OpcService / ModbusService 共享基类的模板方法模式，仅服务于自备外部网关的场景）；两者通过 `isTauri()` 探测自动选择，上层代码无感知。
...
/** 工业协议类型：浏览器无法直连（原生 TCP），Tauri 桌面运行时统一走 Rust 网关 */
...
function getDataService(
  ...
  if (!sourceUrl) return defaultDataService ?? (defaultDataService = new MockDataService())
  ...
  // Tauri 桌面运行时：工业协议统一经 Rust 原生网关（IPC）
  ...
  switch (sourceType) {
    ...
    default:          return defaultDataService ?? (defaultDataService = new MockDataService())
  }
...
开发阶段不可能每次都接真实 PLC。`mock/` 目录下的模拟服务为前 4 类浏览器协议生成了具有**数据特征**的模拟值；三种工业协议的演示模式则由 Rust 侧的 `DemoAdapter` 提供（同样经 IPC 推送，链路和真实设备完全一致）：
...
| S7 / OPC UA / Modbus | 无本地端口 | Rust DemoAdapter | 与真实链路同构 |
...
`npm run dev` 一条命令，四类模拟服务随开发服务器一起启动，零额外操作；生产构建（`npx tauri build`）不携带 mock 服务。
...
节点组件
  ...
  ├── binding.sourceType = 's7'         →  桌面：IpcGatewayService → IPC → Rust 网关 → TCP → PLC
  └── binding.sourceType = 'modbus'     →  浏览器：ModbusService → WS → 外部网关（兜底）
          │
          ▼
