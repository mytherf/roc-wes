...
**实时监控**：统一管理 7 种协议的数据源连接，支持演示/真实双模式一键切换。节点通过 `binding` 配置关联数据源点位，支持 transform 函数将原始值映射为业务状态。事件告警采用 PLC 语义的边沿触发（比较类条件仅在越限瞬间报一次），避免持续报警。
...
└─────────────┼───────────────────────────────────────────┘
   ...
   ┌─────────────────────┐        ┌───────────────────────┐
   │  真实 PLC / 网关    │        │  内置演示引擎          │
   │  Modbus / S7 / OPC  │        │  （Rust DemoAdapter） │
   └─────────────────────┘        └───────────────────────┘
```

前 4 种协议（WebSocket / HTTP / SSE / MQTT）由 WebView 原生支持，前端 Service 直连服务端即可。后 3 种工业 TCP 协议（S7comm / OPC UA / Modbus TCP）WebView 无法直连——由 `IpcGatewayService` 通过 Tauri IPC（invoke 命令 + 事件推送）接入同进程内的 Rust 网关，网关用 tokio 原生 TCP 连接真实 PLC 或演示适配器；全部协议的演示模式均由 Rust DemoAdapter 生成模拟曲线，无需任何本地端口或外部服务。
...
节点通过属性面板的「数据绑定」标签页配置 `pointId` + `sourceType` + `transform` 函数。绑定后，数据点更新自动写入 `node.data.value`，并驱动节点事件评估（值变化触发、比较类条件上升沿触发告警）。
...
│   ├── platform/               # IPC 设备配置转换 / 文件落盘 / 路线独立窗口
...
├── templates/                  # 画面模板（asrs-warehouse / sorting-line 等）
└── docs/                       # 文档中心（用户手册 / 开发指南 / 数据源手册 / articles）
```
...
npx tauri dev
# → 前端 Vite 固定监听 http://localhost:1420（与 tauri.conf.json devUrl 联动）
