 ...
 * 因此通过 WebSocket 网关桥接（内置 mock 见 mock/server.ts 的 startModbusServer）。
 * 订阅协议与 WebSocket / S7 / OPC UA 服务一致：发送 { action, topic } 订阅/取消数据点（寄存器地址），
 * 网关周期性回推 { topic, value, timestamp, quality }。
 *
 * 生产环境可将网关替换为真实的 Modbus-WebSocket 网关（如 modbus-serial + ws 桥接），
 * 前端代码无需改动。
 */
