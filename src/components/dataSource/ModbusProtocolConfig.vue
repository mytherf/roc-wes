<!--
  Modbus 协议专属参数子组件（注册表驱动，见 protocolConfigRegistry.ts）
  真实设备模式的完整接入配置：设备地址（主机[:端口]，
  由 ProtocolAddressInput 公共组件填写）+ 从站地址(unitId) + 轮询间隔。
  直接接收父组件的响应式 form 对象，v-model 原地修改对应字段。
-->
<template>
  <div class="ds-proto-cfg">
    <ProtocolAddressInput :form="form" placeholder="如：192.168.0.10 或 192.168.0.10:502" />
