<!--
  WebSocket 协议配置子组件（注册表驱动，见 protocolConfigRegistry.ts）
  真实设备模式仅需服务地址（Rust 网关作为 WS 客户端连接），
  无额外协议专属参数。
  直接接收父组件的响应式 form 对象，v-model 原地修改对应字段。
-->
<template>
  <div class="ds-proto-cfg">
    <ProtocolAddressInput :form="form" placeholder="如：ws://192.168.0.10:9000/ws" />
  </div>
</template>
