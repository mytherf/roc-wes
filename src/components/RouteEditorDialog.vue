<!-- ══════════════════════════════════════════════════════════════════════
     RouteEditorDialog.vue - 路线编辑器（画布上“画路径”的工具）

     功能一览：
       1. 路线管理：新建 / 选中 / 重命名 / 删除路线（数据存于 routeStore）
       2. 航点绘制：开启“添加航点”模式后在画布上依次点击，生成路径点
       3. 航点编辑：拖拽航点、右键菜单（插入/删除/设为站点/插入中间点）、
          坐标数字输入框微调、清空全部
       4. 分段配置：每段可单独设置速度、通行方向（→/←/↔）、到达停顿时间
       5. 动画预览：小圆点沿路径运动，支持直线/贝塞尔曲线平滑、循环
       6. 画布联动：选中路线高亮（加粗+光晕，其他路线淡化）、视口聚焦到路线

     实现要点：
       - 覆盖层单元格（线段/航点/坐标标签）带 __route_editor_ 前缀，
         id 中加入“渲染代号”，避免同 id 先删后加造成旧视图残留
       - 只在路线 tab 激活时接管画布交互（active prop 控制）
     ══════════════════════════════════════════════════════════════════════ -->
<template>
